...
describe('Authentication API', async () => {
    ...
    it('should register a new user when request is ok', () => {
      return request(app)
        ...
        .then((res) => {
          ...
          expect(res.body.result[0].token).to.have.a.property('accessToken');
          expect(res.body.result[0].token).to.have.a.property('refreshToken');
          expect(res.body.result[0].token).to.have.a.property('expiresIn');
          expect(res.body.result[0].user).to.include(user);
        });
    });
  ...
  //         expect(res.body.result[0].token).to.have.a.property('refreshToken');
  //         expect(res.body.result[0].token).to.have.a.property('expiresIn');
  //         expect(res.body.result[0].user).to.include(dbUser);
  //       });
  //   });
  ...
  //       .expect(httpStatus.OK)
  //       .then((res) => {
  //         expect(res.body.result[0]).to.have.a.property('accessToken');
  //         expect(res.body.result[0]).to.have.a.property('refreshToken');
  //         expect(res.body.result[0]).to.have.a.property('expiresIn');
  //       });
  //   });
